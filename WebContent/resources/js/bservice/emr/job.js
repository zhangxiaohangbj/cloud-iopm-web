define(['Common','bs/modal','jq/form/wizard','bs/tooltip','jq/form/validator-bs3','bs/switcher'],function(Common,Dialog){
	...
	var init = function(){
		...
		//先获取数据，进行加工后再去render
		Common.render(true,'tpls/bservice/emr/job/list.html',function(){
			bindEvent();
		});
	};
	...
	var bindEvent = function(){
		Common.initDataTable($('#networkTable'),{
		      ...
		      "sAjaxSource":"resources/data/bservice/emr/job.txt", //ajax源，后端提供的分页接口
			...
			$tar.prev().find('.left-col:first').append(
					'<span class="btn btn-add">创建Job</span>'
				);
