define(['Common','bs/modal','jq/form/wizard','bs/tooltip','jq/form/validator-bs3','bs/switcher'],function(Common,Dialog){
	...
	var init = function(){
		...
		//先获取数据，进行加工后再去render
		Common.render(true,'tpls/bservice/emr/plugin/list.html',function(){
			bindEvent();
		});
	};
	...
	var bindEvent = function(){
		Common.initDataTable($('#networkTable'),{
		      ...
		      "sAjaxSource":"resources/data/bservice/emr/plugin.txt?", //ajax源，后端提供的分页接口
		      ...
		      "columns": [
			        ...
			        {"data": "description"},
			        {
			        	"defaultContent":'<a class="btn-edit" data-toggle="tooltip" title="编辑集群" href="javascript:void(0)" data-act="stop">查看</a>'
			        }
		      ],
		      /*
		       * columnDefs 属性操作自定义列
